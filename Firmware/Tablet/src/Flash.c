...
void flash_write(uint8_t *data, uint16_t block, uint16_t sector, uint16_t page){
    ...
    uint32_t address = flash_get_address(0,0,16);
...
void flash_test(void){
    ...
    flash_init();
    //flash_block_erase(0);
    //flash_write(&flash_data_array, 0, 0, 0);
    //flash_read(8, spi_rx, 0, 0, 16);
